<template>
  <view>
    <input-area @transRequst="getValue" :ifTextareaShow="ifTextareaShow"></input-area>
...
<script>
  ...
  import resultPopup from './components/result-popup'
    ...
    data() {
      return {
        appid: '20190207000264410',
        key: 'PPiouk6OcxMqJ6j0QiO0',
        form: {
        ...
        currentlanguage: '',
        transBackResult: '',
        ifTextareaShow: false, // 文本框是否可见
      }
    },
    methods: {
      showTextarea (data) {
        this.ifTextareaShow = data
      ...
      transBack (index) {
        ...
        let query = this.form.result[index];
        let from = this.form.language[index];
      ...
      translate() {
        ...
        let key = this.key
        let salt = (new Date).getTime();
        let query = this.form.content;
        // 多个query可以用\n连接  如 query='apple\norange\nbanana\npear'
        let from = this.form.sourceLanguage;
        ...
        for (let lg of this.form.language) {
          uni.request({
            ...
            success:(res)=> {
              this.form.result.push(res.data.trans_result[0].dst)
              this.form.resultFrom = res.data.from
              this.form.resultTo.push(res.data.to)
              console.log(this.form.resultFrom)
            }
          })
        }
      },
      ...
      getValue(sourceLanguage, language, content){
        this.form.sourceLanguage = sourceLanguage
